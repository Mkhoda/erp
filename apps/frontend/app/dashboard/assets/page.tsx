...
import { Pencil, Trash2, List, LayoutGrid, Plus, Boxes, X, Image as ImageIcon, Search } from 'lucide-react';
...
type Asset = {
  ...
  purchaseDate?: string; cost?: number; type?: { id: string; name: string };
  category?: { id: string; name: string }; images?: Array<{ id: string; url: string; caption?: string }>;
};
...
export default function AssetsPage() {
  React.useEffect(() => { document.title = 'مدیریت دارایی‌ها | Arzesh ERP'; }, []);
  ...
  React.useEffect(() => {
    load();
    fetch(`${API}/asset-types`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).then(d => setAssetTypes(Array.isArray(d) ? d : [])).catch(() => setAssetTypes([]));
    fetch(`${API}/asset-categories`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).then(d => setAssetCategories(Array.isArray(d) ? d : [])).catch(() => setAssetCategories([]));
  }, []);

  const q = query.toLowerCase();
  const filtered = assets.filter(a => (a.name || '').toLowerCase().includes(q) || (a.barcode || '').toLowerCase().includes(q));

  function onAdd() { setEditing({ id: '0', name: '', barcode: '', condition: 'NEW', availability: 'AVAILABLE', barcodeType: 'QR' } as any); setSelectedImages([]); setOpen(true); }
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault(); if (!editing || !editing.name || !editing.barcode) return;
    setLoading(true);
    try {
      ...
      setOpen(false); setEditing(null); setSelectedImages([]); await load();
    } catch { alert('خطا در ذخیره دارایی'); } finally { setLoading(false); }
  }

  const avLabel = (a?: string) => AVAILABILITY_LABELS[a || ''] || { label: a || '-', cls: 'bg-theme-secondary text-theme-muted border-theme' };
        ...
        <div className="card-theme-body">
          <div className="flex flex-wrap justify-between items-center gap-4">
            <div className="flex items-center gap-3">
              <div className="flex justify-center items-center bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl w-10 h-10">
                <Boxes className="w-5 h-5 text-white" />
              </div>
              <div>
                <h1 className="font-bold text-theme-primary text-xl">مدیریت دارایی‌ها</h1>
                <p className="text-theme-muted text-sm">{assets.length} دارایی در سیستم</p>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <div className="flex bg-theme-secondary border border-theme p-1 rounded-xl">
                <button onClick={() => setView('list')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${view === 'list' ? 'bg-theme-card shadow text-theme-primary' : 'text-theme-muted hover:text-theme-secondary'}`}>
                  <List className="w-3.5 h-3.5" /> فهرست
                </button>
                <button onClick={() => setView('grid')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${view === 'grid' ? 'bg-theme-card shadow text-theme-primary' : 'text-theme-muted hover:text-theme-secondary'}`}>
                  <LayoutGrid className="w-3.5 h-3.5" /> کارت
                </button>
              </div>
              <button onClick={onAdd} className="btn-theme-primary text-sm gap-1.5">
                <Plus className="w-4 h-4" /> افزودن دارایی
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="card-theme-body py-3">
          <div className="flex gap-3 items-center">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="جستجو نام یا بارکد..." value={query} onChange={e => setQuery(e.target.value)} className="input-theme pr-10 text-sm" />
            </div>
            <span className="text-theme-muted text-xs shrink-0">{filtered.length} دارایی</span>
          </div>
        </div>
      </div>
      ...
      {view === 'list' ? (
        <div className="table-theme-container">
          <div className="overflow-x-auto">
            <table className="table-theme">
              <thead><tr><th>بارکد</th><th>نام</th><th>نوع</th><th>دسته</th><th>وضعیت</th><th>مکان</th><th>اقدامات</th></tr></thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={7} className="py-12 text-center text-theme-muted">هیچ دارایی یافت نشد</td></tr>
                ) : filtered.map(a => {
                    ...
                    <tr key={a.id}>
                      ...
                      <td>
                        <div className="flex gap-2">
                          <button onClick={() => onEdit(a)} className="btn-theme-secondary text-xs py-1 px-2.5"><Pencil className="w-3 h-3" /> ویرایش</button>
                          <button onClick={() => onDelete(a.id)} className="btn-theme-danger text-xs py-1 px-2.5"><Trash2 className="w-3 h-3" /></button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        <div className="gap-4 grid sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map(a => {
            const av = avLabel(a.availability);
            return (
              <div key={a.id} className="card-theme group">
                <div className="card-theme-body">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => onEdit(a)} className="btn-theme-secondary text-xs flex-1 justify-center py-1.5"><Pencil className="w-3 h-3" /> ویرایش</button>
                    <button onClick={() => onDelete(a.id)} className="btn-theme-danger text-xs py-1.5 px-3"><Trash2 className="w-3 h-3" /></button>
                  </div>
                </div>
              </div>
            );
          })}
          {filtered.length === 0 && <div className="col-span-3 py-16 text-center text-theme-muted">هیچ دارایی یافت نشد</div>}
        </div>
      )}
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            onClick={e => { if (e.target === e.currentTarget) { setOpen(false); setSelectedImages([]); } }}>
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
              className="bg-theme-primary shadow-2xl border border-theme rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center px-6 py-4 border-theme border-b">
                <h3 className="font-semibold text-theme-primary">{editing?.id && editing.id !== '0' ? 'ویرایش دارایی' : 'افزودن دارایی جدید'}</h3>
                <button onClick={() => { setOpen(false); setSelectedImages([]); }} className="hover:bg-theme-hover p-1.5 rounded-lg text-theme-muted"><X className="w-4 h-4" /></button>
              </div>
              ...
              <form onSubmit={onSubmit} className="px-6 py-5 space-y-5">
                <div className="gap-4 grid grid-cols-1 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <input value={editing?.oldBarcode || ''} onChange={e => setEditing(s => s ? { ...s, oldBarcode: e.target.value } : s)} className="input-theme font-mono" placeholder="بارکد قدیم" dir="ltr" />
                  </div>
                  <div>
                    <label className="block mb-1.5 font-medium text-theme-secondary text-sm">شماره سریال</label>
                    <input value={editing?.serialNumber || ''} onChange={e => setEditing(s => s ? { ...s, serialNumber: e.target.value } : s)} className="input-theme" placeholder="شماره سریال" dir="ltr" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <label className="flex flex-col items-center justify-center border-2 border-dashed border-theme rounded-xl p-6 cursor-pointer hover:bg-theme-hover transition-colors">
                    ...
                    <input type="file" multiple accept="image/*" onChange={e => setSelectedImages(prev => [...prev, ...Array.from(e.target.files || [])])} className="hidden" />
                  </label>
                    ...
                    <div className="mt-3 gap-2 grid grid-cols-4">
                      {selectedImages.map((f, i) => (
                        <div key={i} className="relative">
                          ...
                          <button type="button" onClick={() => setSelectedImages(p => p.filter((_, j) => j !== i))} className="-top-1.5 -right-1.5 absolute bg-red-500 hover:bg-red-600 rounded-full w-5 h-5 flex items-center justify-center text-white">
                ...
                <div className="flex justify-end gap-3 pt-2 border-theme border-t">
                  <button type="button" onClick={() => { setOpen(false); setSelectedImages([]); }} className="btn-theme-secondary text-sm">انصراف</button>
                  <button type="submit" disabled={loading || uploading} className="btn-theme-primary text-sm disabled:opacity-50">
