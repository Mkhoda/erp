  ...
  async create(userId: string, dto: any) {
    ...
    return this.prisma.attendanceRequest.create({
      data: {
        ...
        gregDate,
        requestedIn: dto.clearCheckIn ? null : buildInstant(dto.date, dto.inTime),
        requestedOut: dto.clearCheckOut ? null : buildInstant(dto.date, dto.outTime),
        clearCheckIn: !!dto.clearCheckIn,
    ...
    if (decision === 'APPROVE') {
      await this.prisma.attendanceOverride.create({
        data: {
          ...
          clearCheckIn: req.clearCheckIn,
          clearCheckOut: req.clearCheckOut,
          forceStatus: req.targetStatus,
          leaveMinutes: req.leaveMinutes,
  ...
  async adminOverride(adminId: string, dto: any) {
    ...
    await this.prisma.attendanceOverride.create({
      data: {
        userId: dto.userId,
        gregDate,
        newCheckIn: dto.clearCheckIn ? null : buildInstant(dto.date, dto.inTime),
        newCheckOut: dto.clearCheckOut ? null : buildInstant(dto.date, dto.outTime),
        clearCheckIn: !!dto.clearCheckIn,
        clearCheckOut: !!dto.clearCheckOut,
